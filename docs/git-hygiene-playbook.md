...
**This repo is self-verifying.** It ships its own `run-tests.sh` at the root — 169 assertions covering `bin/land.sh`'s behaviour against throwaway git repos, plus a lint sweep over every shell script here — and `run-tests.sh` is the first thing land.sh's detection ladder looks for. So landings in *this* repo really do run tests, including the landing of a change to land.sh itself. Run it directly any time: `./run-tests.sh` (~20s, no network).
