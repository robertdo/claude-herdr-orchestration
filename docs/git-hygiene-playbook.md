...
**The system in one line:** every change happens in a herdr worktree on a branch, landed as a single squash-merged commit — so main stays a clean, linear history by construction, not by discipline. Landing via `bin/land.sh` runs tests as part of that path; the Bash guard (`git-hygiene-guard.sh`) is a best-effort linter, not enforcement, so a command spelled around it still reaches main unverified. For enforcement that does not depend on command spelling or which tool issued it, see the opt-in `hooks/git-repo/reference-transaction` guard. See "What this does and does not enforce" below.
...
Four layers exist, in increasing order of strength, and it matters which one you're relying on:

- **Bash guard** (`git-hygiene-guard.sh`) — a best-effort linter. Pre-execution, catches the accidental case, gives a good message. Defeated by spelling the command differently.
- **Edit guard** (`git-hygiene-edit-guard.sh`) — sound within its scope: it resolves the target through git rather than parsing text. Its limits are tool scope, parent-directory classification, and textual `cwd` comparison — not string-parsing.
- **`reference-transaction` guard** (`hooks/git-repo/reference-transaction`) — structural: runs inside git itself, immune to command spelling and to which tool issued the operation. Opt-in per repository. Enforces linearity and single-commit advance, **not provenance** — same-uid removable, so it's not a security boundary either, just a stronger floor. Full detail: `README.md`'s ["The per-repository git guard"](../README.md#the-per-repository-git-guard).
- **Remote branch protection** (server-side, squash-only merges) — the only layer that is an actual privilege boundary against a same-uid adversary. Not implemented by this repo.

All four are **guardrails against careless agents, not a security boundary against adversarial ones** (remote branch protection is the exception, and this repo doesn't implement it). That's not a small caveat, but it's not a reason to skip the local ones either: catching a plain `git commit` on main and a direct edit to the primary checkout, pre-execution, with a good error message, at zero workflow cost, is genuinely valuable — it stops the two most common ways a long session drifts.

**Does:** deny, before execution, a direct `git commit` on `main`/`master` (outside an empty repo's first commit — there is no squash-merge exception; the blessed landing path, `bin/land.sh`, never writes `SQUASH_MSG`, so exempting it was both forgeable and dead) and a direct file edit inside the primary checkout, on any branch.

**Does not — the commit guard (`git-hygiene-guard.sh`, Bash) is structurally unsound, not merely buggy.** It decides whether a command is a forbidden git operation by pattern-matching the literal command string, and the input language is Turing-complete shell — "does this string eventually move `refs/heads/main`" is undecidable in general. Verified bypasses that remain (not an exhaustive list — see the hook's own header comment, which is the fix point for parser cases, and item 4 there: chasing this list further is explicitly out of scope):
...
Two cases that used to be on this list are fixed, not just documented around: `git -c key=value commit` (the flag-group regex now has a dedicated `-c <value>` alternative, since that flag takes its value as a separate token and agents pass it routinely) and the `touch .git/SQUASH_MSG` two-step (the exception it relied on is gone — see the asymmetry note below). Even with both fixed, a linter that reads command text can still misfire on some future case; the correct response to a false denial is to use the sanctioned path (`bin/land.sh`), not to disable the hook.

**The deliberate asymmetry, so it doesn't read as a bug:** the Bash linter now denies a manual `git merge --squash` + `git commit` on main just like any other direct commit — but the `reference-transaction` guard, if installed, would *permit* that same sequence, because a squash-merge onto an unchanged tip is a single-parent fast-forward, which is exactly the shape it allows. These are different jobs: the Bash guard gives pre-execution guidance ("use `bin/land.sh`"), while `reference-transaction` enforces a *shape* at the ref regardless of how it was produced. Neither claims to detect *provenance* — which script or command built the commit — only the linter tries to guess intent from spelling, and only the git hook enforces structure.
...
**The tool surface is not closed.** The Claude Code hooks are `PreToolUse` matchers scoped to specific tools — `Bash` for the commit guard, `Edit`/`Write`/`NotebookEdit` for the edit guard. Any other tool that can run a shell command or write a file bypasses both, completely: an MCP server exposing file or shell tools (e.g. serena's `create_text_file`, `replace_content`, `execute_shell_command`), `sed -i`, shell redirects, `patch`, `git apply`, formatters, and build scripts never reach either guard. (`reference-transaction` closes exactly this gap for the commit side — see below.)

**The Claude Code hooks can disarm themselves.** The edit guard exempts `~/.claude/**` — the exact directory the hooks and `settings.json` live in. One `Edit` to the guard script, or to `settings.json`, turns off enforcement, effective immediately, in the same session.
...
- **`hooks/git-repo/reference-transaction` is implemented, opt-in per repository** (`install-repo.sh`; `install.sh` does not touch it — this repo does not install it into itself, since a wrong version here would reject the very landings needed to fix it). It runs inside git itself, so it's immune to command-spelling tricks and sees every tool that ultimately invokes `git` — closing the tool-surface gap above. It is still deletable or edit-around-able by any process running as the same user, so it does not close the self-disarm gap, and it enforces linearity/single-commit-advance, not provenance (see the asymmetry note above). Full detail, configuration, and the escape hatch: `README.md`'s ["The per-repository git guard"](../README.md#the-per-repository-git-guard).
- **Remote branch protection** with squash-only merges enforced server-side is the only boundary a same-uid local process can't remove — it's the actual privilege boundary if you need one, and this repo does not implement it.
...
**The `send-keys Enter` after `herdr pane run <pane> "<full task prompt>"` is required, not optional, when the prompt contains newlines.** A multi-line prompt is *pasted* into the pane but not *submitted* — it sits in the input box as `[Pasted text #1 +N lines]` and the worker never starts, while `herdr pane run` itself still reports success, so nothing in that command's own output tells you dispatch failed. The follow-up `herdr pane send-keys <pane> Enter` submits it. This cost real time in this repo's own history — confirm the worker actually reached `working` by reading the `herdr wait` **output**, not its exit status: `herdr wait` exits `0` even on a timeout, so a `||` fallback chained onto it never fires.

**Worker sizing: two independent dials (model + effort).** The orchestrator sizes each dispatch on model (capability) and effort (reasoning depth) separately — it has the task description and no stake in doing the work itself, so this judgment belongs here. **Size the worker on both dials, explicitly.** Dispatch the cheapest model that clears the task's hardest requirement (importance ≠ difficulty — most work does not need your top-tier model; reserve that for the genuinely hard tail), and set effort to match reasoning depth (`high` is a sane default; `xhigh` only for the hardest architecture/debug). Pass `--model` and `--effort` explicitly — your harness default is not the right dispatch default. This sizing is one application of the [model-routing framework](model-routing.md) — the same two dials (capability tier × reasoning effort), here aimed at a dispatched worker; the author's filled-in ladder is the worked [example](model-routing-example.md).
...
1. **Rebase automatically if needed.** The branch must contain main; if it doesn't, `bin/land.sh` rebases it onto main's tip inside the branch's own worktree — an isolated checkout, so a conflict there can never touch main or the primary checkout — before anything is tested or built. A conflict leaves the rebase **in progress** in the worktree: resolve it there, finish the rebase, then re-run. That isolation is what keeps conflict resolution, abort, and recovery entirely out of the landing path itself.
...
**This repo is self-verifying.** It ships its own `run-tests.sh` at the root — hundreds of assertions covering `bin/land.sh`'s behaviour, `hooks/git-repo/reference-transaction` (installed only into throwaway fixtures — never into this repo, see the guard's own test above), and `hooks/git-hygiene-guard.sh` (driven directly over stdin, no Claude Code session needed), plus a lint sweep over every shell script here — and `run-tests.sh` is the first thing land.sh's detection ladder looks for. So landings in *this* repo really do run tests, including the landing of a change to land.sh or the guards themselves. Run it directly any time: `./run-tests.sh` (~30s, no network).
